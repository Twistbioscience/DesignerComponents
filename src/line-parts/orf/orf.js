...
const splitSequenceIntoChunks = (sequence, startIndex, endIndex, firstBrickType, lastBrickType) => {
    ...
    const translationStartIndex = firstBrickType === 2 ? startIndex + 1 : (firstBrickType === 1 ? startIndex - 1 : startIndex);
...
const getBricksData = (orfs, charsPerRow, letterWidth, endIndex, orfLineHeight, sequence) => orfs.map((orf, index, arr) => {
    ...
    const textChunks = splitSequenceIntoChunks(sequence, orf.orfLineStart, orf.orfLineEnd, firstBrickType, lastBrickType);
    return {
        ...orf,
        start,
        firstBrickType,
