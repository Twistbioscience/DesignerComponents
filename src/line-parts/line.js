...
class Line extends React.Component {
  ...
  render() {
    ...
    const getRect = () => {
      const startX =
        selection.startIndex && selection.startIndex > startIndex
          ? (selection.startIndex - startIndex) * config.OTHER_LETTERS_WIDTH
          : 0;
      const endX = selection.endIndex
        ? selection.endIndex < endIndex
          ? (selection.endIndex - startIndex) * config.OTHER_LETTERS_WIDTH
          : (endIndex - startIndex) * config.OTHER_LETTERS_WIDTH
        : -1;
    ...
    return (
      <svg
        style={style}
        width={config.OTHER_LETTERS_WIDTH * charsPerRow}
