...
export const detectOrfs = ({ sequence = '', startCodons = ['ATG'], minAALength = MIN_ORF_SIZE }) => {
  ...
  const { starters, complementStarters } = startCodons.reduce((acc, startCodon) => ({
  ...
  const { stoppers, complementStoppers } = STOP_CODON.reduce((acc, stopCodon) => ({
  ...
  starters.forEach(function(starter) {
      orfs = orfs.concat(detectOrfsInDnaSequence(starter, stoppers, sequence, minAALength, pairs, startCodons, firstRelevantStopper, 'forward'));
  });
  pairs = {};
  firstRelevantStopper = { index : 0 };
  complementStarters.forEach(function(starter) {
      orfs = orfs.concat(detectOrfsInDnaSequence(starter, complementStoppers, sequence, minAALength, pairs, startCodons, firstRelevantStopper, 'reverse'));
  });
...
/**
     ...
     * @return {Array}
     */
    function detectOrfsInDnaSequence(startIndex, stoppers, dnaSequence, minAALength, pairs, startCodons, firstRelevantStopper, strand) {
        const singleAALength = 3;
        var orfs = [], stopIndex, diff;
